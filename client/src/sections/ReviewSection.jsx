...
const ReviewSection = ({ blogId }) => {
    ...
    return (
        <div className="flex w-full relative flex-col gap-y-20  rounded-md px-10 " >
            ...
            <div className="flex gap-y-2  gap-x-8 w-full overflow-x-auto bg-teal-100 px-3 py-6 md:flex-row">
                {
                    !reviews.length > 0 ? <h3 className="text-xl font-semibold text-center">No Reviews Yet</h3>
                        ...
                        reviews?.map(review => (
                            ...
                            <div key={review._id} className={` ${review?.user?._id === user?.userId ? "order-first bg-blue-500 " : ""} flex w-9/12 relative rounded-md flex-col items-start gap-y-3 md:gap-y-6 p-2 md:p-4 bg-blue-200 hover:bg-blue-400 md:min-w-1/3 min-10/12  h-full`}>
                                ...
                                <p className="text-sm ">{review.review.length > 15 ? `${review.review.split(0, 10)}...` : review.review}</p>
                            </div>
