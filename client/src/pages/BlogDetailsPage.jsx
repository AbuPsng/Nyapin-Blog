...
const BlogDetailsPage = () => {
    ...
    const { handleGetSingleBlog, blog, handleDeleteBlog } = useBlog()
        ...
        <Template>
            <main className="pt-10 pb-20 flex flex-col items-center gap-y-7" >
                <div className="pt-20 pb-12 w-full flex flex-col gap-y-14">
                    <img className="w-full h-80 object-cover" src={`${blog?.coverImage}`} alt="" />
                    <div className="relative">
                        {blog?.author?._id === user?.userId && <div className="absolute right-2 top-2 ">
                            ...
                            <button onClick={() => { handleDeleteBlog(blog?._id), navigate("/") }} className="py-2 px-7 rounded-md bg-red-200 hover:bg-red-400">delete</button>
                        </div>}
                        <div className="flex flex-col gap-y-4 py-10 px-5">
                            ...
                            <p className="text-sm pt-1">Created At</p>
                        </div>
                    </div>
                </div>
