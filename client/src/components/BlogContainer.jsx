...
const BlogContainer = ({ blog, handleDelete }) => {
    const { user } = useUser()
        ...
        <div className="relative py-4 flex flex-col gap-y-3 w-full">
            {
                user?.userId === blog?.author ?
                    <div className="absolute right-0 top-4 rounded-lg " >
                        ...
                        <button onClick={() => handleDelete(blog._id)} className="font-semibold bg-red-400 rounded-lg md:py-2 md:px-3 "><MdDelete /></button>
                    </div>
                    :
                    null
            }
